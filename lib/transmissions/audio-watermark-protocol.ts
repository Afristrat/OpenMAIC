...
export function encodeAudioWatermarkMessages(
  watermarkId: string,
): readonly number[] {
  assertWatermarkId(watermarkId);
  return encodePayloads(watermarkId).map(
    (payload, index) => (index << 12) | payload,
  );
}
 ...
 */
export function decodeAudioWatermarkMessages(
  messages: Iterable<number>,
): string | null {
  const payloads = new Map<number, number>();
  for (const message of messages) {
    if (!Number.isInteger(message) || message < 0 || message > 0xffff)
      return null;
