 ...
 * - Cartesia: https://docs.cartesia.ai/api-reference/tts/bytes (Sonic 3)
 ...
 *
 * The API route maps this class to HTTP 429; downstream workers can also use
 * it to apply retry/backoff without parsing provider-specific messages.
 */
...
async function generateAzureTTS(
  ...
  // Derive locale from the voice ID (e.g. "fr-FR-DeniseNeural" → "fr-FR")
...
async function generateFishAudioTTS(
  ...
  if (!response.ok) {
    throwIfTtsRateLimited('Fish Audio', response.status);
    const errorText = await response.text().catch(() => response.statusText);
