...
const resultSchema = z.object({
  success: z.literal(true), status: z.literal('queued'), deliveryId: z.uuid(),
...
export function saveLtiAttempt(scope: string, answers: QuizAnswers, language: string): LtiQuizAttempt {
  ...
  if (localStorage.getItem(key(scope)) !== serialized) throw new Error('LTI attempt storage unavailable');
  return attempt;
}
export function clearLtiAttempt(scope: string): void { localStorage.removeItem(key(scope)); }
...
export async function readLtiClassroomContext(stageId: string, signal: AbortSignal) {
  const response = await fetch(`/api/lti/context?stageId=${encodeURIComponent(stageId)}`, {
    cache: 'no-store', credentials: 'same-origin', signal: AbortSignal.any([signal, AbortSignal.timeout(15000)]),
  });
  if (!response.ok) throw new Error('LTI context unavailable');
  return z.discriminatedUnion('active', [
    z.object({ success: z.literal(true), active: z.literal(false) }),
    z.object({ success: z.literal(true), active: z.literal(true), gradingEnabled: z.literal(true), launchId: z.uuid() }),
  ]).parse(await response.json());
}

/** Replaying uses exactly the saved answers and request ID, never a browser score. */
export async function sendLtiQuizAttempt(stageId: string, sceneId: string, attempt: LtiQuizAttempt, signal: AbortSignal) {
