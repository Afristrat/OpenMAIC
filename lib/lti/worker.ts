...
const jobSchema = z.object({
  ...
  line_item_url: z.string().url(),
  score: z.coerce.number().min(0).max(100),
  score_changed_at: z.string(),
});
...
export async function deliverNextLtiGrade(): Promise<boolean> {
  ...
  const [platform, user, resource, organization] = await Promise.all([
    ...
    service
      ...
      .select('user_id,lms_subject')
      .eq('id', job.user_binding_id)
      .eq('org_id', job.org_id)
      .maybeSingle(),
    service
      .from('lti_resource_bindings')
      .select('resource_link_id')
      .eq('id', job.resource_binding_id)
      .eq('org_id', job.org_id)
      .maybeSingle(),
    service.from('organizations').select('status').eq('id', job.org_id).maybeSingle(),
  ]);
  if (user.error || resource.error || organization.error)
    throw new Error('LTI delivery scope lookup failed');
  let result: GradeDeliveryResult = {
    success: false,
    retryable: false,
    error: 'LTI delivery access inactive',
  };
  if (platform && user.data && resource.data && organization.data?.status === 'active') {
    result = await submitGrade(
      platform,
      job.line_item_url,
      {
        userId: user.data.lms_subject,
      ...
      {
        qalemUserId: user.data.user_id,
        resourceLinkId: resource.data.resource_link_id,
  ...
  if (completed.error || completed.data !== true)
    throw new Error('LTI delivery acknowledgement failed');
  return true;
}
  ...
  const tick = () => {
    ...
    running = (async () => {
      try {
        ...
        log.error('LTI delivery iteration failed; durable lease retained');
      }
    })().finally(() => {
      running = undefined;
    });
