...
export async function gradeLtiQuiz(content: unknown, submittedAnswers: unknown, language: string) {
  ...
  for (const question of questions) {
    ...
    if (question.type === 'short_answer') {
      ...
      const selected =
        answer === undefined || answer === '' ? [] : Array.isArray(answer) ? answer : [answer];
      if (
    ...
    if (answer?.trim()) {
      ...
      const prompts = buildQuizGradePrompts({
        ...
        commentPrompt: [
          ...
          question.answer?.length ? `Reference answer: ${question.answer.join('; ')}` : undefined,
        ]
          .filter(Boolean)
          .join('\n'),
        language,
  ...
  const score =
    Math.round((ordered.reduce((sum, result) => sum + result.earned, 0) / total) * 10000) / 100;
