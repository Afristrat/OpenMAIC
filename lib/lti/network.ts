...
export class LtiNetworkPolicyError extends Error {
  constructor() { super('LTI endpoint or response rejected'); }
}
...
function publicAddress(address: string): boolean {
  ...
  return !!family && !isPrivateIP(address) &&
    (family === 4 ? !reserved.check(address, 'ipv4') : globalV6.check(address, 'ipv6') && !reserved.check(address, 'ipv6'));
}

export type LtiRequest = { method: 'GET' | 'POST'; headers?: Record<string, string>; body?: string };

/** Resolve once, pin the TLS connection, bound decoded bytes, and release every socket. */
export async function requestLtiEndpoint(endpoint: string, init: LtiRequest, maxBytes: number): Promise<Response> {
  ...
  try {
    ...
    const response = await fetchDirect(url, {
      ...init, dispatcher: agent, redirect: 'manual', signal: controller.signal,
    });
    ...
    return new Response(response.status === 204 || response.status === 205 ? null : Buffer.concat(chunks).toString('utf8'), {
