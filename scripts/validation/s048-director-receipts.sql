...
BEGIN
 ...
 EXCEPTION WHEN check_violation THEN NULL; END;
 IF NOT EXISTS(SELECT 1 FROM qalem_telemetry_private.director_receipts WHERE director_receipts.id IN (proof.id,proof.id2)) THEN RAISE EXCEPTION 'Contractual receipts erased'; END IF;
 IF jsonb_array_length(public.read_account_director_export_page(actor,'director_receipts'))<>2 THEN RAISE EXCEPTION 'Contractual export changed'; END IF;
