...
WHERE user_id='00000000-0047-4000-8000-000000000021';
...
BEGIN
 ...
 -- Reclaim an ungraded first attempt, then remove only its analytical evidence.
 claim := public.begin_classroom_quiz_attempt(actor,org,'s047-link-proof','s047-link-scene',gen_random_uuid(),'{"q":"a"}');
 DELETE FROM public.discussion_patterns WHERE id=d_id;
 IF EXISTS(SELECT 1 FROM public.discussion_patterns WHERE id=d_id) OR
   EXISTS(SELECT 1 FROM public.classroom_quiz_attempts WHERE discussion_pattern_id=d_id) THEN RAISE EXCEPTION 'Analytical cleanup retained linkage'; END IF;
 PERFORM public.complete_classroom_quiz_attempt(actor,(claim->>'id')::uuid,(claim->>'leaseId')::uuid,grade);
 IF (SELECT result FROM public.classroom_quiz_attempts WHERE id=(claim->>'id')::uuid) IS NULL THEN RAISE EXCEPTION 'Functional correction blocked by analytical cleanup'; END IF;
 IF public.read_account_discussion_export_page(actor,'discussion_patterns')<>'[]'::jsonb THEN RAISE EXCEPTION 'Cleaned export retained'; END IF;
 IF public.read_authorized_discussion_patterns(actor,org,ARRAY['s047-link-proof'],'SIPOC','fr-FR')<>'[]'::jsonb THEN RAISE EXCEPTION 'Cleaned aggregate retained'; END IF;
