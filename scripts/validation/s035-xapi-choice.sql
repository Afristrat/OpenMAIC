...
BEGIN
 ...
 IF public.record_consented_learning(actor,gen_random_uuid(),'s035-course-xapi',payload,epoch,org) THEN RAISE EXCEPTION 'Old consent epoch survived'; END IF;
 SELECT collection_epoch INTO epoch FROM public.telemetry_consent WHERE user_id=actor;
 PERFORM public.record_consented_learning(actor,gen_random_uuid(),'s035-course-xapi',payload,epoch,org);
 IF EXISTS(SELECT 1 FROM public.xapi_outbox WHERE org_id=org) THEN RAISE EXCEPTION 'xAPI refusal allowed course projection'; END IF;
