...
BEGIN
 ...
 payload := '{"scene_sequence":["quiz"],"scene_durations":[12],"quiz_scores":[1],"total_duration":12,"completion_rate":1,"agent_count":1,"scene_observations":[{"id":"s035-x-quiz","type":"quiz","seconds":12,"completed":true,"score":1,"attempts":[0,1,1]}]}'::jsonb;
 ...
 IF jsonb_array_length(statements)<>6 THEN RAISE EXCEPTION 'Lost repeated attempts'; END IF;
 ...
 FOREACH invalid IN ARRAY ARRAY['null'::jsonb,'{}'::jsonb,'[null]'::jsonb,'["1"]'::jsonb,'[2]'::jsonb,'[0]'::jsonb,'[]'::jsonb] LOOP
   BEGIN
     PERFORM public.record_consented_learning(actor,gen_random_uuid(),'s035-course-xapi',
       jsonb_set(payload,'{scene_observations,0,attempts}',invalid),epoch,org);
 ...
 IF NOT EXISTS(SELECT 1 FROM public.pedagogy_telemetry WHERE session_id=session) THEN RAISE EXCEPTION 'xAPI withdrawal erased internal analytics'; END IF;
 RAISE NOTICE 'Repeated quiz attempts, rejection, export, replay and withdrawal verified';
END $$;
RESET ROLE;
