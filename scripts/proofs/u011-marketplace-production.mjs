...
const labels = JSON.parse(await readFile(new URL('../../lib/i18n/locales/ui-fr-FR.json', import.meta.url), 'utf8'));
...
let complete = false;
const configuration = {
  name: 'Recette U-011 — agent temporaire', role: 'student', persona: 'Analyse une situation professionnelle.',
  avatar: '/avatars/teacher-2.png', color: '#112233', priority: 9,
  allowedActions: ['wb_open'], gender: 'female', interactionWeight: 37,
...
try {
  ...
  const membership = await admin.from('org_members').insert({ user_id: userId, org_id: orgId, role: 'author' });
  assert.ok(!membership.error, 'Unable to create proof author membership');
  const link = await admin.auth.admin.generateLink({ type: 'magiclink', email });
  assert.ok(!link.error && link.data.properties?.hashed_token, 'Unable to create private login link');
  ...
  await context.addCookies(parts.map((value, index) => ({
    name: parts.length === 1 ? key : `${key}.${index}`, value,
    domain: 'qalem.ma', path: '/', secure: true, sameSite: 'Lax',
  })));
  const page = await context.newPage();
  await page.addInitScript((agent) => {
    localStorage.setItem('locale', 'fr-FR');
    localStorage.setItem('agent-registry-storage', JSON.stringify({ version: 11, state: { agents: {
      'u011-local': { ...agent, id: 'u011-local', isDefault: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    } } }));
  }, configuration);
  ...
  await page.getByRole('combobox', { name: labels['marketplace.publicationOrg'], exact: true }).selectOption(orgId);
  await page.getByRole('combobox', { name: labels['marketplace.localAgent'], exact: true }).selectOption('u011-local');
  const before = await admin.from('agent_configs').select('id', { count: 'exact', head: true }).eq('owner_id', userId);
  ...
  await dialog.getByLabel(labels['marketplace.publicDescription'], { exact: true }).fill('Recette temporaire automatisée U-011.');
  const saved = page.waitForResponse((response) => new URL(response.url()).pathname === '/api/marketplace/agents/drafts' && response.request().method() === 'POST');
  ...
  assert.equal((await context.request.get(`${baseUrl}/api/marketplace/agents/${snapshotId}`)).status(), 404);
  await page.reload();
  await expect(owned.getByText(privateLabel, { exact: true })).toBeVisible();
  await owned.getByRole('button', { name: labels['marketplace.publish'], exact: true }).click();
  await expect(owned.getByText(publishedLabel, { exact: true })).toBeVisible();
  const rows = await admin.from('agent_configs').select('id,is_published,org_id').eq('owner_id', userId);
  assert.ok(!rows.error && rows.data.length === 1 && rows.data[0].id === snapshotId && rows.data[0].is_published && rows.data[0].org_id === orgId, 'Republication must reuse exactly one tenant snapshot');
  complete = true;
} finally {
  const cleanupErrors = [];
  if (session) {
    try {
      ...
      if (logout.error || !refresh.error || refresh.data.session) cleanupErrors.push('session revocation');
    } catch { cleanupErrors.push('session revocation'); }
  }
  await browser?.close();
  if (userId) {
...
assert.ok(complete);
console.log(JSON.stringify({ publication: true, withdrawal: true, reload: true, republication: true, foreignTenantStatus: 403, temporaryFixturesDeleted: true, sessionFileCreated: false }));
