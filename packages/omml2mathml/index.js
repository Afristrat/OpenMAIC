/**
 * Derived from scienceai/omml2mathml 1.3.0 (Apache-2.0).
 * Modified by Qalem: replace get-dom/jsdom with @xmldom/xmldom so the
 * converter no longer depends on the obsolete request HTTP stack.
 */
...
module.exports = function omml2mathml(omml) {
  ...
  return walker
    ...
    .match(m.el('m:oMathPara'), (src, out, w) => {
      setup(src, out, w);
    ...
    .match(m.el('m:f'), (src, out, w) => {
      let type = (selectAttr('./m:fPr[last()]/m:type', 'm:val', src) || '').toLowerCase(),
        outer = type === 'lin' ? el('mrow', {}, out) : el('mfrac', fracProp(type), out);
    ...
    .match(m.el('m:sSubSup'), (src, out, w) => {
    ...
    .match(m.el('m:m'), (src, out, w) => {
      let mcjc = selectAttr(
          'm:mPr[last()]/m:mcs/m:mc/m:mcPr[last()]/m:mcJc',
          'm:val',
          src,
        ).toLowerCase(),
        outer = el('mtable', mcjc && mcjc !== 'center' ? { columnalign: mcjc } : {}, out);
      select('m:mr', src).forEach((mr) => {
        let mtr = el('mtr', {}, outer);
        select('m:e', mr).forEach((me) => {
          let mtd = el('mtd', {}, mtr);
          w.walk(mtd, me);
        });
      });
    })
    .match(m.el('m:rad'), (src, out, w) => {
    ...
    .match(m.el('m:groupChr'), (src, out, w) => {
    ...
    .match(m.el('m:borderBox'), (src, out, w) => {
      let hideTop = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:hideTop[last()]', 'm:val', src).toLowerCase(),
        ),
        hideBot = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:hideBot[last()]', 'm:val', src).toLowerCase(),
        ),
        hideLeft = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:hideLeft[last()]', 'm:val', src).toLowerCase(),
        ),
        hideRight = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:hideRight[last()]', 'm:val', src).toLowerCase(),
        ),
        strikeH = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:strikeH[last()]', 'm:val', src).toLowerCase(),
        ),
        strikeV = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:strikeV[last()]', 'm:val', src).toLowerCase(),
        ),
        strikeBLTR = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:strikeBLTR[last()]', 'm:val', src).toLowerCase(),
        ),
        strikeTLBR = forceTrue(
          selectAttr('m:borderBoxPr[last()]/m:strikeTLBR[last()]', 'm:val', src).toLowerCase(),
        ),
        outer;
      if (
        hideTop &&
    ...
    .match(m.el('m:phant'), (src, out, w) => {
      let zeroWid =
        selectAttr('m:phantPr[last()]/m:zeroWid[last()]', 'm:val', src).toLowerCase() || false;
      if (zeroWid) zeroWid = forceFalse(zeroWid);
      let zeroAsc =
        selectAttr('m:phantPr[last()]/m:zeroAsc[last()]', 'm:val', src).toLowerCase() || false;
      if (zeroAsc) zeroAsc = forceFalse(zeroAsc);
      let zeroDesc =
        selectAttr('m:phantPr[last()]/m:zeroDesc[last()]', 'm:val', src).toLowerCase() || false;
      if (zeroDesc) zeroDesc = forceFalse(zeroDesc);
      let showVal = forceFalse(
        selectAttr('m:phantPr[last()]/m:show[last()]', 'm:val', src).toLowerCase(),
      );
      let parent;
      if (showVal) {
...
function fracProp(type) {
  ...
  return {};
  // The original XSLT referenced undefined numerator/denominator alignment variables;
  // omitting those attributes preserves its effective behavior.
}
...
function setup(src, out, w) {
  if (w.res) return;
  let doc = new DOMImplementation().createDocument(MATH_NS, null, null),
    nod = nodal(doc, {}, nsMap),
