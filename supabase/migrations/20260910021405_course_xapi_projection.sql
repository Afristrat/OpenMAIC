-- The existing outbox owns delivery. A course record owns its local xAPI lifecycle.
ALTER TABLE public.xapi_outbox ADD COLUMN learning_observation_id uuid
  REFERENCES public.pedagogy_telemetry(id) ON DELETE CASCADE;
CREATE INDEX xapi_outbox_learning_observation ON public.xapi_outbox(learning_observation_id)
  WHERE learning_observation_id IS NOT NULL;

CREATE FUNCTION qalem_telemetry_private.project_course_xapi()
RETURNS trigger LANGUAGE plpgsql SECURITY INVOKER SET search_path = '' AS $$
DECLARE
  destination text; scene jsonb; result jsonb; verbs text[]; verb text;
BEGIN
  ...
  FOR scene IN SELECT value FROM jsonb_array_elements(NEW.scene_observations) LOOP
    verbs := ARRAY[]::text[];
    ...
    FOREACH verb IN ARRAY verbs LOOP
      ...
      VALUES(NEW.org_id,NEW.id,'course:' || NEW.id::text || ':' || (scene->>'id') || ':' || verb,
        destination,jsonb_build_object(
          ...
          'context',jsonb_build_object('contextActivities',jsonb_build_object('parent',jsonb_build_array(
            ...
            'extensions',jsonb_build_object('https://qalem.ma/xapi/scene-type',scene->>'type',
              'https://qalem.ma/xapi/measurement','end-of-session-scene-summary')),
...
CREATE TRIGGER project_course_xapi AFTER INSERT ON public.pedagogy_telemetry
  FOR EACH ROW EXECUTE FUNCTION qalem_telemetry_private.project_course_xapi();

-- Recheck immediately before HTTP. A previously started network request is not revocable.
CREATE FUNCTION public.authorize_xapi_delivery(p_id bigint)
RETURNS boolean LANGUAGE sql STABLE SECURITY INVOKER SET search_path = '' AS $$
  SELECT EXISTS (
    ...
    WHERE x.id=p_id AND x.status<>'sent' AND (
      x.learning_observation_id IS NULL OR EXISTS (
...
-- Filter before LIMIT: revoked/disabled rows must not starve eligible deliveries.
CREATE FUNCTION public.list_due_xapi_deliveries()
RETURNS TABLE(id bigint) LANGUAGE sql STABLE SECURITY INVOKER SET search_path='' AS $$
  SELECT x.id FROM public.xapi_outbox x
    JOIN public.organization_lrs_configs c ON c.org_id=x.org_id AND c.enabled AND c.endpoint=x.lrs_target
  WHERE x.status<>'sent' AND x.next_attempt_at<=now() AND public.authorize_xapi_delivery(x.id)
  ORDER BY x.id LIMIT 100;
$$;
REVOKE ALL ON FUNCTION public.list_due_xapi_deliveries() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.list_due_xapi_deliveries() TO service_role;
