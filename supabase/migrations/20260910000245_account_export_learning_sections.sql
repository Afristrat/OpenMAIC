...
BEGIN
  ...
  CASE p_section
    ...
    WHEN 'course_imports' THEN v_table := 'course_imports'; v_key := 'id'; v_scope := 't.owner_id=$1 AND NOT EXISTS(SELECT 1 FROM public.courses c WHERE c.import_id=t.id AND c.org_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM public.org_members m JOIN public.organizations o ON o.id=m.org_id AND o.status=''active'' WHERE m.org_id=c.org_id AND m.user_id=$1))';
      v_fields := string_to_array('id,owner_id,original_filename,storage_path,canvas_version,validation_status,validation_report,created_at',',');
  ...
  IF v_fields IS NULL THEN
    CASE v_table
      ...
      WHEN 'pedagogy_telemetry' THEN v_fields := string_to_array('id,user_hash,subject_hash,session_id,org_id,stage_id,scene_sequence,scene_durations,quiz_scores,completion_rate,total_duration,subject_tags,language,level,agent_count,action_counts,created_at',',');
    END CASE;
  END IF;
