...
const LOCALE_CASES = [
  {
    ...
    planHeading: /plan de formation/i,
    confirmPlan: /confirmer et générer le cours/i,
  },
  {
    locale: 'ar-MA',
    dir: 'rtl',
    ...
    planHeading: /مخطط التكوين/i,
    confirmPlan: /تأكيد وتوليد المقرر/i,
  },
] as const;

test.describe('Generation flow — locale coverage (S0-008)', () => {
  for (const { locale, dir, requirement, heading, planHeading, confirmPlan } of LOCALE_CASES) {
    test(`home → persistent generation in ${locale}`, async ({ page, mockApi }) => {
      ...
      await expect(page.getByRole('heading', { name: planHeading })).toBeVisible();
