...
test('publishes a private snapshot explicitly and withdraws it after reload', async ({ page }) => {
  const configuration = {
    name: 'Analyste de recette', role: 'student', persona: 'Analyse les situations.',
  ...
  await page.addInitScript((agent) => {
    ...
    localStorage.setItem('agent-registry-storage', JSON.stringify({
      version: 11, state: { agents: { 'local-agent': {
        ...agent, id: 'local-agent', isDefault: false,
  ...
  await page.route('**/api/marketplace/agents/owned?*', (route) => route.fulfill({ json: {
    success: true, agents: saved ? [{ id: 'server-snapshot', name: configuration.name, published }] : [],
  ...
  await dialog.getByLabel('Description publique', { exact: true }).fill('Analyse de situations professionnelles.');
  expect(saved).toBe(false);
