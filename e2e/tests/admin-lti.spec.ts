...
for (const [locale, labels] of Object.entries({ 'fr-FR': fr, 'ar-MA': ar, 'en-US': en })) {
  test(`LTI platform persists only after server success (${locale})`, async ({ page, browserConsoleContract }) => {
    ...
    await page.route('**/api/lti/platforms', async (route) => {
      if (route.request().method() === 'GET') return route.fulfill({ json: platform ? [platform] : [] });
      writes++;
    ...
    const values = {
      'admin.lti.orgId': '00000000-0034-4000-8000-000000000002', 'admin.lti.clientId': 'test-lms-client',
      'admin.lti.issuer': 'https://lms.example', 'admin.lti.jwksUrlField': 'https://lms.example/jwks',
      'admin.lti.authUrl': 'https://lms.example/auth', 'admin.lti.tokenUrl': 'https://lms.example/token',
    ...
    await expect(page.getByRole('alert').filter({ hasText: labels['admin.lti.saveFailed'] })).toBeVisible();
    await expect(page.getByText('(test-lms-client)', { exact: true })).toBeHidden();
    await expect(page.getByLabel(labels['admin.lti.clientId'], { exact: true })).toHaveValue('test-lms-client');
    await page.getByRole('button', { name: labels['admin.lti.save'], exact: true }).click();
