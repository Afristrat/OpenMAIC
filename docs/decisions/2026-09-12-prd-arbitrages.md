...
| A6 — AudioSeal | Oui, sous condition : AudioSeal reste la preuve de provenance ; un blocage hors Qalem n’est retenu que s’il est réellement démontré. Les contenus téléchargés doivent être réservés à un usage local, jamais à une interface web. | Aucun blocage externe ne sera promis par AudioSeal seul : un fichier téléchargé peut toujours être copié. Le respect technique d’un usage local exige un client contrôlé et un mécanisme de diffusion distinct, à cadrer et prouver dans une story dédiée ; les replays et transmissions Qalem restent en streaming authentifié, sans téléchargement. |
