# S-034 — Intégration LTI/AGS clôturée

État constaté le 9 septembre 2026, code applicatif `674e6cc`, infrastructure déployée `8b995bd`, branche `refork-v030`. S-034 est clôturée après recette réelle et import au coffre autorisé.
...
La panne volontaire a transitoirement rendu la santé Moodle `unhealthy` sans OOM ni redémarrage. Le premier moniteur a donc refusé de commencer sa fenêtre saine. Retour à `healthy` confirmé, puis session **63893 terminée, sortie 0** : **10:31:37–10:46:53 UTC**, 31 relevés sains sur cinq conteneurs et deux endpoints HTTP 200, aucun OOM ni redémarrage, `S034_RUNTIME_HEALTH_15MIN_OK`. Contrôle complémentaire des cgroups web/worker/capture : `max=0`, `oom=0`, `oom_kill=0`. Les preuves sont poussées dans `e08447ed28f5db4bb90644c1dbe2a71e4f1da960` ; le diff depuis `8b995bd` ne modifie pas le code applicatif. Auto-déploiement web et worker relu `false`, aucun nouveau déploiement déclenché par ce push documentaire.

## Conditions de clôture et limites

1. LMS choisi et installé après le « Oui feu vert » d’Amine du 9 septembre : Moodle 4.5.13, https://lms-test.qalem.ma. Installation officielle et connexion administrateur réelle prouvées, sans inscription libre. Source officielle figée au SHA `8cbae18a2898cfd8266ec91ac206e12004f0ff5f`, images par digest, base/réseau/volumes dédiés ; web 2 Gio et DB 512 Mio sans swap. Procédure : `infra/moodle/README.md`. La recette LTI distincte est détaillée ci-dessus.
2. Observation post-déploiement et persistance des preuves acquises par les contrôles ci-dessus, comme le déploiement, les clés runtime et la recette LMS fonctionnelle.
...
4. **Import terminé après accord explicite** : session 93305 exit 0, six entrées ajoutées via `add-secret.ps1 -Value`, sauvegarde et revalidation loader officielles. Aucun remplacement, aucune rotation, aucun passage par le presse-papier ni impression de valeur. Les quatre mots de passe Moodle et les deux PEM LTI (base64 sur une ligne, dont la clé publique non confidentielle) sont inventoriés dans `infra/moodle/README.md`. Un processus frais via le broker confirme six égalités exactes avec les fichiers serveur. Le premier échec de presse-papier reste un fait historique, pas un prérequis ouvert.

Dernière revalidation après import : carnet Moodle lu dans Chromium (A=0, B=100), trois livraisons `sent` avec 1/1/2 tentatives et quatre événements d’audit, dont trois succès. Web et worker servent toujours le SHA `8b995bd`, sains sans OOM ni redémarrage. Le runner reste sur ce SHA, sans diff applicatif ; son dernier résultat Playwright est `passed`, sans échec. Aucun nouveau gate global ni déploiement n’a été lancé pour un simple import de coffre ; les gates datés du même code restent ceux cités plus haut.

`status=completed`, `passes=true` et les preuves datées sont consignés dans le PRD. Cela clôt S-034, pas l’ensemble du PRD ni les validations humaines restantes.
