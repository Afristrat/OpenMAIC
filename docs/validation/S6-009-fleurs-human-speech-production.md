...
Cette preuve établit une transcription Qalem fonctionnelle sur trois langues et des voix humaines autorisées, sur des extraits courts et longs. Elle ne clôt pas S6-009 : le microphone réel et ses refus de permission, ainsi que la panne amont visible dans l’interface, restent à effectuer. La topologie effective LiteLLM Hostinger → DGX par Tailscale est attestée séparément dans `S6-009-asr-chain-smoke.md` ; Cloudflare ne fait pas partie du chemin ASR. L’arabe mesuré est de l’arabe standard ; aucune conclusion n’est tirée sur le darija marocain ni sur la qualité TTS.
