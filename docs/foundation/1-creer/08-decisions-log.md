...
- **Preuves SCORM 2004 et cmi5** : la recette S1-008 du 28 août 2026 importe et exécute le paquet SCORM 2004 dans Moodle, relit `completed` et `passed`, puis obtient `Conformant` dans ADL CATAPULT avec la séquence cmi5 `initialized` → `completed` → `terminated` ; voir `docs/validation/S1-008-scorm2004-cmi5.md`.
