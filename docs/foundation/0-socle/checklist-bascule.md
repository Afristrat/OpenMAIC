...
3. **Quatre divergences de câblage identifiées spécifiquement sur `refork-v030`** (fichier porté, mais jamais importé/rendu dans l'app, contrairement à `main`) : le composant certificat en fin de classroom (U-007, `certificate-prompt.tsx` absent de `stage.tsx`), l'indicateur de synchronisation (U-019, `offline-indicator.tsx` absent de `app/layout.tsx`), la bannière d'installation PWA (U-020, `pwa-install-banner.tsx` absent de `app/layout.tsx`) et **la bannière de consentement télémétrie RGPD/CNDP** (U-021, `telemetry-consent-banner.tsx` absent de `app/layout.tsx` — point sensible de conformité).
...
| U-008 | Page Mes certificats | Page listant les certificats obtenus | `app/certificates/page.tsx` | PRÉSENT sur refork (fichier présent, accessible même si le déclencheur U-007 est cassé côté classroom) | Oui. | |
...
| U-010 | Page Plugins de scènes | Page listant les plugins de scènes disponibles | `app/plugins/page.tsx` | PRÉSENT identique sur refork | Oui — cohérent avec S-044/S-045/S-046 confirmés câblés dans le rendu réel des scènes. | |
| U-011 | Publier un agent sur la marketplace | Dialogue de publication d'un agent depuis le registry local | `components/agent/publish-agent-dialog.tsx` | PRÉSENT identique sur refork | Oui. | |
...
| Story ID | Titre | Capacité (résumé) | Preuve main (fichier:ligne) | État sur refork-v030 | Utilisée en prod sur main ? | Décision |
|---|---|---|---|---|---|---|
| U-019 | Indicateur sync dans sidebar | Indicateur de statut de synchronisation online/offline | `components/offline-indicator.tsx` importé et rendu dans `app/layout.tsx:14,59` | **DIVERGENT** — `components/offline-indicator.tsx` existe sur disque (copié par S0-002) mais **la liste complète des imports de `app/layout.tsx` du refork ne contient pas `OfflineIndicator`**, et une recherche exhaustive du repo ne trouve la référence que dans le fichier lui-même. Composant orphelin, jamais rendu dans le layout réel. | Oui sur main (visible sur toutes les pages via le layout racine). | |
