...
4. **FSRS : étendre le porté, pas le réécrire** : `lib/spaced-repetition/` copié-adapté est la référence ; le lien entre `compiled-plan.schema.json` et les items se traite par story dédiée, pas en refonte opportuniste.
