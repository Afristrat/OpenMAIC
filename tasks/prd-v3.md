...
- Le registre actif contient 76 US : 41 sont acceptées sur preuve et 35 restent réellement ouvertes.
- Le reliquat réel se répartit en 16 validations, 16 décisions préalables et 3 blocages ; aucune US ne reste en `to_implement`.
- Dix des 26 US encore étiquetées `to_validate` portent déjà `passes=true` et doivent uniquement être normalisées en `completed` après contrôle de cohérence.
