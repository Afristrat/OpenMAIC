...
export function LtiBindings({
  ...
  async function assign() {
    ...
    try {
      ...
      const row = z
        .object({ id: z.uuid(), orgId: z.uuid() })
        .parse(await mutate('/api/lti/platforms', 'PATCH', { platformId, orgId: expected }));
      if (row.id !== platformId || row.orgId.toLowerCase() !== expected)
  ...
  return (
    <div className="pt-2">
      <Button
        ...
        disabled={saving}
      >
        {t('admin.lti.manageBindings')}
      </Button>
      {open && (
        <div id={formId} className="space-y-4 pt-3">
          {!orgId ? (
            <form
              onSubmit={(event) => {
...
function BindingList({
    ...
    void fetch(`/api/lti/bindings?${params}`, {
      ...
      .then(async (response) => {
        ...
        const result = z
          .object({
            orgId: z.uuid(),
            bindings: z.array(z.unknown()).max(100),
            nextOffset: z.number().int().nonnegative().nullable(),
          })
          .parse(await response.json());
        if (
          result.orgId !== orgId ||
          (result.nextOffset !== null && result.nextOffset !== offset + 100)
        )
          throw new Error('Invalid LTI page');
  ...
  async function create() {
    ...
    try {
      const row = readBinding(
        await mutate('/api/lti/bindings', 'POST', {
          platformId,
          kind,
          ...(kind === 'resource'
  ...
  async function revoke(binding: Binding) {
    if (
      saving ||
      !window.confirm(`${t('admin.lti.revokeConfirm')}\n${binding.source}\n${binding.target}`)
    )
      return;
    setSaving(true);
    setWriteFailed(false);
    try {
      z.object({ success: z.literal(true) }).parse(
        await mutate('/api/lti/bindings', 'DELETE', {
          platformId,
          kind,
  ...
  return (
    <section aria-labelledby={`${formId}-title`} className="rounded-lg border p-3 space-y-3">
      <h4 id={`${formId}-title`} className="font-medium">
        {t(kind === 'resource' ? 'admin.lti.resourceBindings' : 'admin.lti.userBindings')}
      </h4>
      <Button
        variant="outline"
        size="sm"
      ...
      >
        {t('admin.lti.reloadBindings')}
      </Button>
      {loadFailed ? (
        <p role="alert">{t('admin.lti.loadFailed')}</p>
      ) : !page ? (
        <p role="status">{t('admin.lti.loadingBindings')}</p>
      ) : (
        <>
          {page.bindings.length === 0 ? (
            <p>{t('admin.lti.noBindings')}</p>
          ) : (
            <ul className="space-y-2">
              {page.bindings.map((binding) => (
                <li key={binding.id} className="flex flex-wrap items-center gap-2 border-b pb-2">
                  ...
                  <Button
                    ...
                    aria-label={`${t('admin.lti.revokeBinding')} ${binding.source}`}
                  >
                    {t('admin.lti.revokeBinding')}
                  </Button>
                </li>
              ))}
            </ul>
          ...
          <div className="flex gap-2">
            ...
            >
              {t('admin.lti.previousBindings')}
            </Button>
            <Button
              variant="ghost"
              size="sm"
              disabled={saving || page.nextOffset === null}
              onClick={() => {
                if (page.nextOffset !== null) setOffset(page.nextOffset);
              }}
            >
              {t('admin.lti.nextBindings')}
            </Button>
          </div>
        </>
      )}
        ...
        <fieldset disabled={saving} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label htmlFor={`${formId}-source`}>
              {t(kind === 'resource' ? 'admin.lti.resourceLinkId' : 'admin.lti.lmsSubject')}
            </label>
            <Input
              id={`${formId}-source`}
              value={source}
          ...
          <div>
            <label htmlFor={`${formId}-target`}>
              {t(kind === 'resource' ? 'admin.lti.stageId' : 'admin.lti.userId')}
            </label>
            <Input
              id={`${formId}-target`}
              value={target}
