...
describe('widget template persistence migration (S6-027)', () => {
  it('persists global templates, immutable versions and publication audits', () => {
    ...
    expect(migration).toMatch(
      /BEFORE UPDATE OR DELETE ON public\.widget_template_versions/i,
    );
  });
  ...
  it('allows authenticated users to read only published templates and versions', () => {
    ...
    expect(migration).toMatch(
      /ENABLE ROW LEVEL SECURITY[\s\S]+widget_template_versions/i,
    );
    expect(migration).toMatch(
      /FOR SELECT[\s\S]+TO authenticated[\s\S]+published_version_id IS NOT NULL/i,
    );
    expect(migration).toMatch(
      /FOR SELECT[\s\S]+TO authenticated[\s\S]+published_at IS NOT NULL/i,
    );
    expect(migration).toMatch(
      /REVOKE INSERT, UPDATE, DELETE[\s\S]+FROM authenticated/i,
    );
    expect(migration).not.toMatch(
      /FOR (?:INSERT|UPDATE|DELETE|ALL)[\s\S]+TO authenticated/i,
    );
  });
