...
describe('classroom RBAC', () => {
  ...
  it('requires active author membership for marketplace even for a global administrator', async () => {
    ...
    expect(
      (await requireSuperAdminOrOrgAuthor(request, 'org-target', { requireMembership: true }))
        .response?.status,
    ).toBe(403);
    mocks.membership = { role: 'author', organizations: { status: 'active' } };
    expect(
      (await requireSuperAdminOrOrgAuthor(request, 'org-target', { requireMembership: true })).user
        ?.id,
    ).toBe('root-1');
