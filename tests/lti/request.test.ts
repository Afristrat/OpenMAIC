...
describe('bounded LTI form reader', () => {
  ...
  it.each([undefined, '1', '999999'])('bounds actual body bytes despite Content-Length %s', async (length) => {
    const headers: Record<string, string> = length ? { 'content-length': length } : {};
    await expect(readLtiForm(post('state=12345', headers), 10)).rejects.toMatchObject({ status: 413 });
  });
  it('bounds a GET query', async () => {
    await expect(readLtiForm(new NextRequest('https://qalem.ma/api/lti/login?state=12345'), 10))
      .rejects.toMatchObject({ status: 413 });
  });
  it.each(['application/json', 'text/plain', 'application/x-www-form-urlencoded-evil'])('rejects media type %s', async (type) => {
    await expect(readLtiForm(post('state=a', { 'content-type': type }), 1024))
      .rejects.toMatchObject({ status: 415 });
  });
  it.each(['state=a&state=b', 'state=a&%73tate=b', 'state=a%00b', 'sta%0ate=a'])('rejects ambiguous or control parameters', async (body) => {
  ...
  it('cancels a chunked body as soon as it exceeds the cap', async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) { controller.enqueue(new TextEncoder().encode('state=oversized')); }, cancel,
    });
