...
describe('verified LTI launch claims', () => {
  it('preserves the verified identity, nonce and declared AGS permissions', () => {
    ...
    expect(
      parseVerifiedLaunchClaims(
        { ...payload, [LTI_CLAIMS.AGS]: { lineitem: 'https://lms.example.org/items/1', scope } },
        platform,
      ),
    ).toMatchObject({
      userId: 'learner',
  ...
  it('requires the registered authorized party for multiple audiences', () => {
    expect(
      parseVerifiedLaunchClaims({ ...payload, aud: ['client', 'other'], azp: 'client' }, platform)
        .userId,
    ).toBe('learner');
  });

  it.each([
    'http://lms.example.org/item',
    'https://user:password@lms.example.org/item',
    'https://lms.example.org/item#fragment',
    'invalid',
  ])('rejects unsafe endpoint format without echoing it: %s', (lineitem) => {
    expect(() =>
      parseVerifiedLaunchClaims(
        { ...payload, [LTI_CLAIMS.AGS]: { lineitem, scope: [] } },
        platform,
      ),
    ).toThrow('Invalid LTI launch claims');
  });
});
