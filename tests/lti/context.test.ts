...
const session = { id, org_id: id, client_id: 'client', resource_binding_id: id, user_binding_id: id,
  expires_at: '2099-01-01T00:00:00Z', line_item_url: 'https://lms.example/lineitem',
  ags_scopes: ['https://purl.imsglobal.org/spec/lti-ags/scope/score'] };
function query(data: unknown, error: object | null = null) {
  const result = { select: vi.fn(), eq: vi.fn(), maybeSingle: vi.fn().mockResolvedValue({ data, error }) };
  result.select.mockReturnValue(result); result.eq.mockReturnValue(result); return result;
}
function allow() {
...
describe('LTI classroom capability authorization', () => {
  ...
  it('hashes the cookie and binds access to current Auth user and requested classroom', async () => {
    ...
    expect(lookup.eq).toHaveBeenCalledWith('token_hash', createHash('sha256').update(input.token).digest('hex'));
  ...
  it.each([null, { ...session, expires_at: '2000-01-01T00:00:00Z' }])('rejects missing or expired launches', async (data) => {
  ...
  it.each([0, 1, 2, 3])('refuses failed access predicate %i before reading quiz content', async (index) => {
    const { checks } = allow();
  ...
  it('reads only the quiz scene of the authorized classroom', async () => {
    allow(); const scene = query({ content: { type: 'quiz', questions: [] } });
    mocks.from.mockReturnValueOnce(scene);
    expect((await loadLtiQuiz(input, 'quiz')).content).toEqual({ type: 'quiz', questions: [] });
    expect(scene.eq.mock.calls).toEqual([['id', 'quiz'], ['stage_id', 'bound-stage'], ['type', 'quiz']]);
  });
