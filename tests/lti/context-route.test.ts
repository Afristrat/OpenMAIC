...
describe('LTI context route', () => {
  beforeEach(() => { vi.resetAllMocks(); mocks.auth.mockResolvedValue({ user: { id: 'verified-user' } }); });
  ...
  it('returns no identity, endpoint, token or answer key and prevents caching', async () => {
    mocks.resolve.mockResolvedValue({ gradingEnabled: true, orgId: 'private', sessionId: 'private' });
    const result = await GET(request());
    expect(await result.json()).toEqual({ success: true, active: true, gradingEnabled: true });
    expect(result.headers.get('cache-control')).toBe('private, no-store');
    expect(mocks.resolve).toHaveBeenCalledWith({ token: 'a'.repeat(64), userId: 'verified-user', stageId: 'stage' });
  });
  it.each([[new LtiAccessDenied(), 403], [new Error('private error'), 503]] as const)('does not silently downgrade failed LTI context to ordinary grading', async (error, status) => {
