...
describe('LTI platform lookup failures', () => {
  ...
  it.each([getPlatformConfig, getPlatformConfigByIssuer])(
    'distinguishes missing registrations from database failures',
    async (lookup) => {
  ...
  it('maps the registered platform without exposing unrelated columns', async () => {
    ...
    expect(await getPlatformConfig('client')).toEqual({
      id: 'id',
      clientId: 'client',
      issuer: 'issuer',
      jwksUrl: 'jwks',
