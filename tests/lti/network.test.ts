...
describe('LTI pinned public HTTPS transport', () => {
  ...
  it.each([
    ...
    'https://u:p@lms.example/key',
    'https://lms.example/key#hash',
    'https://localhost/key',
    'https://127.0.0.1/key',
    'https://10.0.0.1/key',
    'https://100.64.0.1/key',
    'https://169.254.169.254/key',
    'https://224.0.0.1/key',
    'https://[::1]/key',
    'https://[::ffff:127.0.0.1]/key',
    'https://[64:ff9b::a00:1]/key',
    'https://[2002:0a00:0001::]/key',
  ...
  it('rejects a mixed public/private DNS answer instead of selecting its safe entry', async () => {
    ...
    await expect(
      requestLtiEndpoint('https://lms.example/key', { method: 'GET' }, 100),
    ).rejects.toThrow('LTI endpoint');
    expect(mocks.fetch).not.toHaveBeenCalled();
  });
  it('pins the socket lookup to the verified answer, preserving hostname and TLS validation', async () => {
    ...
    if (!connect || typeof connect !== 'object' || !('lookup' in connect) || !connect.lookup)
  ...
  it.each([302, 307])(
    'refuses redirect %s without following or forwarding credentials',
    async (status) => {
      mocks.fetch.mockResolvedValue(
        new Response(null, { status, headers: { Location: 'https://127.0.0.1/private' } }),
      );
      await expect(
        requestLtiEndpoint('https://lms.example/token', { method: 'POST', body: 'fixture' }, 100),
      ).rejects.toThrow('LTI endpoint');
  ...
  it('bounds actual decoded bytes regardless of claimed content length and closes the agent', async () => {
    mocks.fetch.mockResolvedValue(
      new Response('x'.repeat(101), { headers: { 'Content-Length': '1' } }),
    );
    await expect(
      requestLtiEndpoint('https://lms.example/key', { method: 'GET' }, 100),
    ).rejects.toThrow('LTI endpoint');
