...
vi.mock('@/lib/lti/quiz-submission', async (original) => ({
  ...(await original<typeof import('@/lib/lti/quiz-submission')>()),
  submitLtiQuiz: mocks.submit,
}));
...
function request(data: unknown = body, headers: Record<string, string> = {}) {
  return new NextRequest('https://qalem.ma/api/lti/quiz', {
    method: 'POST',
    headers: {
      origin: 'https://qalem.ma',
      'content-type': 'application/json',
      cookie: `lti_context=${'a'.repeat(64)}`,
      ...headers,
    },
...
describe('authenticated LTI quiz route', () => {
  ...
  it.each<Record<string, string>>([{ origin: 'https://foreign.example' }, { cookie: '' }])(
    'refuses foreign origins and missing launch cookies',
    async (headers) => {
  ...
  it('bounds the actual request body even without content-length', async () => {
    expect((await POST(request({ ...body, answers: { q: 'a'.repeat(2097152) } }))).status).toBe(
      413,
    );
    expect(mocks.submit).not.toHaveBeenCalled();
  });
  ...
  it('returns a service failure, never a synthetic grade, after correction failure', async () => {
    ...
    expect(await response.json()).toEqual({
      success: false,
      error: 'LTI quiz submission unavailable',
    });
  });
});
