...
vi.mock('@supabase/ssr', () => ({
  createServerClient: () => ({ auth: { verifyOtp: mocks.verify, signOut: mocks.signOut } }),
}));
const binding = {
  userId: 'qalem-user',
  orgId: 'tenant',
  ...
  userBindingId: 'identity',
  lmsSubject: 'opaque-subject',
};
const launch: LTILaunchContext = {
  ...
  agsScopes: ['score'],
  lineItemUrl: 'https://lms.example.org/item',
};
...
describe('LTI SSO and durable context', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', 'https://db.example.org');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY', 'public-fixture');
    mocks.account.mockResolvedValue({
      data: { user: { id: binding.userId, email: 'registered@example.org' } },
      error: null,
    });
    mocks.link.mockResolvedValue({
      data: { user: { id: binding.userId }, properties: { hashed_token: 'test-otp' } },
      error: null,
    });
    mocks.verify.mockResolvedValue({
      data: { user: { id: binding.userId }, session: {} },
      error: null,
    });
  ...
  it('uses the registered account and stores only the hash of the context cookie', async () => {
    ...
    await establishLaunchSession(
      new NextRequest('https://qalem.ma/api/lti/launch'),
      response,
      binding,
      launch,
    );
  ...
  it('refuses an account switched during link generation', async () => {
    mocks.link.mockResolvedValue({
      data: { user: { id: 'foreign' }, properties: { hashed_token: 'test' } },
      error: null,
    });
    await expect(
  ...
  it('does not issue an LTI cookie when persistence fails and revokes the new session', async () => {
    ...
    await expect(
      establishLaunchSession(new NextRequest('https://qalem.ma'), response, binding, launch),
    ).rejects.toThrow('persistence failed');
