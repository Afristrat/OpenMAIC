...
const storage = { getItem: vi.fn((key: string) => memory.get(key) ?? null),
  setItem: vi.fn((key: string, value: string) => memory.set(key, value)), removeItem: vi.fn((key: string) => memory.delete(key)) };
const fetchMock = vi.fn();
const id = '00000000-0034-4000-8000-000000000001';
const grade = { success: true, status: 'queued', deliveryId: id, score: 75,
  results: [{ questionId: 'q', correct: true, status: 'correct', earned: 3 }] };
describe('LTI quiz browser boundary', () => {
  beforeEach(() => { memory.clear(); vi.clearAllMocks(); vi.stubGlobal('localStorage', storage); vi.stubGlobal('fetch', fetchMock); });
  ...
  it('reuses the same saved attempt after reload and isolates launch scopes', () => {
    ...
    expect(saveLtiAttempt('user:launch:stage:scene', { q: 'B' }, 'en-US').requestId).not.toBe(first.requestId);
  });
  ...
  it('rejects invalid context instead of downgrading to local grading', async () => {
    ...
    fetchMock.mockResolvedValueOnce(Response.json({ success: true, active: true, gradingEnabled: false, launchId: id }));
    await expect(readLtiClassroomContext('stage', new AbortController().signal)).rejects.toThrow();
  });
  it('polls busy correction with identical request ID and answers, without browser scores', async () => {
    ...
    fetchMock.mockResolvedValueOnce(Response.json({ success: true, status: 'grading' }, { status: 202 }))
  ...
  it('keeps the saved attempt on HTTP failure and never returns a fallback grade', async () => {
    ...
    await expect(sendLtiQuizAttempt('stage', 'scene', attempt, new AbortController().signal)).rejects.toThrow();
