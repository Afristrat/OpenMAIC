...
describe('analytics state storage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('distinguishes an unprovisioned account from an unavailable database', async () => {
    mocks.maybeSingle.mockResolvedValue({ data: null, error: null });
  ...
  it('returns the provisioned contractual analytics state', async () => {
    ...
    mocks.maybeSingle.mockResolvedValue({
      data: { pedagogy_consent: false, collection_epoch: epoch },
      error: null,
    });
    expect(await readConsentState('user')).toEqual({ choice: false, epoch });
    mocks.maybeSingle.mockResolvedValue({
      data: { pedagogy_consent: true, collection_epoch: 'invalid' },
      error: null,
    });
