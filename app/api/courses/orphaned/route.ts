...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = db
      .from('courses')
      .select('id, title, language, status, stage_id, owner_id')
      .eq('org_id', parsed.data.orgId)
      .or(
        `owner_id.is.null,and(owner_id.eq.${z.string().uuid().parse(auth.user.id)},status.eq.draft)`,
      )
    ...
    const courses = (data ?? [])
      .slice(0, 50)
      .map(({ owner_id, ...course }) => ({ ...course, owned: owner_id === auth.user.id }));
