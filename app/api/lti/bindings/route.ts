...
const createInput = z.discriminatedUnion('kind', [
  base
    .extend({ kind: z.literal('resource'), resourceLinkId: identifier, stageId: identifier })
    .strict(),
...
const fields = {
  resource: 'id,resource_link_id,stage_id',
  user: 'id,lms_subject,user_id',
} as const;
...
async function platformContext(platformId: string) {
  ...
  const platform = z
    .object({ client_id: identifier, org_id: z.uuid().nullable() })
    .parse(result.data);
  if (platform.org_id) {
    const org = await service
      .from('organizations')
      .select('id')
      .eq('id', platform.org_id)
      .eq('status', 'active')
      .maybeSingle();
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const result = await service
      .from(tables[type])
      .select(fields[type])
      .eq('client_id', platform.client_id)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await service
      ...
      .select(fields[input.kind])
      .single();
    if (result.error)
      throw new LtiAdminRequestError(
