...
const querySchema = z.object({
  page: z.string().regex(/^[1-9]\d{0,5}$/).transform(Number).default(1),
}).strict();
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  const { data, error, count } = await supabase.from('agent_configs')
  ...
  const response = apiSuccess({
    agents: (data ?? []).map((agent) => ({
      id: agent.id, name: agent.name, orgId: agent.org_id,
      published: agent.is_published, description: agent.description, tags: agent.tags ?? [],
    })),
