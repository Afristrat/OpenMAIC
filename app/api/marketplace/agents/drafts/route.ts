...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const { data: saved, error: readError } = await supabase.from('agent_configs')
    .select('id, is_published')
    .eq('id', id).eq('owner_id', auth.user.id).eq('org_id', orgId).maybeSingle();
  if (readError || !saved) return apiError(API_ERROR_CODES.INTERNAL_ERROR, 500, 'Unable to verify private agent');
